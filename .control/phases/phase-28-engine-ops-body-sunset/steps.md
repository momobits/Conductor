...
- [x] 28.3 — Migrate `implement` op to RunArtifactWriter. `implement.ts` writes `<runId>/implement.md`; drops body append at line 137. No downstream op reads `## Implementation Guidelines` (terminal artifact). Update test fixtures. Verify Card Detail view's artifact panel renders all four new artifact kinds (review, verify, notebook, implement) alongside the existing analyze + plan.
