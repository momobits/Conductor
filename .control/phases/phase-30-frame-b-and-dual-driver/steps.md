...
- [ ] 30.12 — Frame B Cohort C: `card-detail-run-history-surface` (#52) via `/relay-auto --sweep all`. Per-op `⋯` history toggle; new `card_runs_list` RPC; section state machine extends with `viewing-history` state. Polish; depends on #47's section state machine. S complexity. Commits use scope `(30.12)`.
