...
- [ ] 30.15 — Frame B Cohort B convergence: `chat-driven-description-authoring` (#49) via `/relay-auto --sweep all`. Extends `ModelAdapter` with `invokeWithTools`; new `src/engine/ops/chat_agent.ts` with 4-tool surface (grep / read / glob / propose-description-edit); diff-preview UI with Apply/Reject buttons; v1 capped at 1 round of tool calls. Depends on #47 ✓, #48 ✓, #62 (lands in 30.14). L complexity → `/relay-superplan` candidate. **Final item in active backlog**; closing this empties the active feature queue. Commits use scope `(30.15)`.
