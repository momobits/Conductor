...
- [ ] 30.3 — Dual-driver foundation continued: `dual-driver-lead-follow-protocol` (#55) via `/relay-auto`. Newly-unblocked priority leader after #54 closed (4 in-cluster dependents: #56, #57, #59, #62). New `src/conductor/lead.ts` module with global single-lead state (`human | llm`) + `transferLead()` + typed SSE events + reason taxonomy. Subsumes the archived Frame B Feature #51 (`brain-halt-on-user-chat`): "user chat halts brain" becomes "operator-takes-lead with reason: 'user-chat'." Pipeline: `/relay-analyze → /relay-plan or /relay-superplan → /relay-review → implement → /relay-verify → /relay-resolve`. Commits use scope `(30.3)` per the Control bridge in CLAUDE.md.
