...
- [ ] 30.13 — Dual-driver Cohort C **BIG-BANG SWITCH**: `dual-driver-brain-loop-replacement` (#59) via `/relay-auto --sweep all`. Replaces `defaultAgentFactory` + `runOneCard`'s column switch with orchestrator-driven dispatch via #54's `decide()` engine. Architecturally central; depends on #54 ✓, #55 ✓, #57 (lands in 30.8), #58 ✓, #60 (lands in 30.7), #61 (lands in 30.10) — all upstream items in this sweep. L complexity → `/relay-superplan` candidate. Commits use scope `(30.13)`.
