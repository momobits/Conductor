...
- [ ] 30.5 — Frame B Cohort A completion: `card-detail-op-controls-and-button-states` (#48) via `/relay-auto`. The other Cohort A item; tied with #47 at 2 in-cluster dependents (#49, #50). Sequenced after #47 in-phase to close the 30.4 v1 caveat: this step lands the `op_invoke` RPC that #47's empty-state CTAs will swap to (the placeholder `card_work` wiring becomes the real `op_invoke` wiring). Also lands new `card_resume` RPC + 4-state Work-button machine (Idle / Running / Halted-by-chat / Halted-by-assist / Pipeline-complete) + per-op sidebar buttons with column-eligibility matrix + card-detail keyboard shortcuts via the global dispatcher. Pipeline: `/relay-analyze → /relay-plan or /relay-superplan → /relay-review → implement → /relay-verify → /relay-resolve`. Commits use scope `(30.5)` per the Control bridge in CLAUDE.md.
