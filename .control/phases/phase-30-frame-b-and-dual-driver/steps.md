...
- [ ] 30.11 — Frame B Cohort C: `column-transition-op-triggering` (#50) via `/relay-auto --sweep all`. Column moves trigger corresponding op per autonomy policy; new `src/ui/lib/column_ops.ts` with COLUMN_OPS_MAP; extends board_dnd + board_keys drop/move handlers. Reads autonomy spectrum from #60. S complexity. Commits use scope `(30.11)`.
