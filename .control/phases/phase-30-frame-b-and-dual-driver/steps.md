...
- [ ] 30.7 — Dual-driver foundation completion: `dual-driver-autonomy-spectrum-config` (#60) via `/relay-auto --sweep all`. Replaces `autonomy.transitions.*` per-edge config with `autonomy: assist | hybrid | autonomous` spectrum + per-mode budgets + legacy migration. 1 in-cluster dependent (#62). Pipeline standard. Commits use scope `(30.7)`.
