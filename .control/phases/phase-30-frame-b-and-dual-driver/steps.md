...
- [ ] 30.6 — Dual-driver foundation continued: `dual-driver-backward-transitions-and-substrate-advisory` (#58) via `/relay-auto`. Tied at 1 in-cluster dependent with #60 (broken by file order in relay-ordering.md). Widens the lifecycle state machine to allow all column→column edges; adds keep/wipe/branch hygiene RPCs for transitions that orphan substrate. Required before #59 brain-loop-replacement (so orchestrator has widened edges available). Pipeline: `/relay-analyze → /relay-plan or /relay-superplan → /relay-review → implement → /relay-verify → /relay-resolve`. Commits use scope `(30.6)` per the Control bridge in CLAUDE.md.
