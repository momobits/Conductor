...
- [x] 30.6 — Dual-driver foundation continued: `dual-driver-backward-transitions-and-substrate-advisory` (#58) via `/relay-auto`. **SHIPPED 2026-05-24** via `/relay-auto` session `2026-05-24-021842` (9-commit pattern: 8 impl commits `6acdaf7`/`7561b0a`/`8c8f4c7`/`fb0467f`/`39d7343`/`c21babb`/`f8edaa4`/`5f2adca` + 1 resolve commit `6625b09`; much finer-grained than prior steps — each commit is a coherent slice). Widens lifecycle state machine to all column→column edges; new `substrate_hygiene` module (orphan detect + wipe + branch); new substrate-hygiene RPC handlers (find/wipe/branch); new `substrate-orphaned` DaemonEvent variant; drag-drop + keyboard-move + new `conductor card backward` CLI all wire into a shared `moveWithAdvisory` helper. v1 UX path = three concurrent surfaces (UI dialog via extended `src/ui/lib/dialog.ts` `chooseSubstrateAdvisory` + CLI + SSE event for brain-loop dispatch). Pipeline: /relay-analyze (7 related-work findings, scope: keep narrow) → /relay-plan (10 steps × 19 files; closer to L than M in actual surface) → /relay-review (APPROVED-WITH-CHANGES, 7 trivial applied) → implement (0 deviations) → /relay-verify (COMPLETE, suite 912 → 945, +33 tests) → /relay-resolve. Per-item agent summary: `.relay/.auto-session/2026-05-24-021842/58.json`. Implementation doc: `.relay/implemented/dual-driver-backward-transitions-and-substrate-advisory.md`. Spec archived: `.relay/archive/features/dual-driver-backward-transitions-and-substrate-advisory.md`.

<!-- ▼ Sweep allocation 30.7–30.15: pre-authored 2026-05-24 for `/relay-auto --sweep all` session 2026-05-24-030632. Dependency-respecting order across the remaining 9 active items. Each row will be flipped [ ] → [x] by the per-item bridge close-out commit. -->
- [x] 30.7 — `dual-driver-autonomy-spectrum-config` (#60) — **SHIPPED 2026-05-24** via sweep session `2026-05-24-030632` (commits `dc2dde2` impl + `0328da9` resolve). Review APPROVED (clean; 0 trivial). Implement: 5 deviations applied + documented. Verify: 2 iterations, 4 fixes applied. Suite 945 → 966 (+21). Spectrum config landed with legacy migration + new `src/conductor/autonomy.ts` helpers + per-mode budgets. Summary: `.relay/.auto-session/2026-05-24-030632/60.json`.
- [x] 30.8 — `dual-driver-lead-handoff-reconciliation` (#57) — **SHIPPED 2026-05-24** via sweep session `2026-05-24-030632` (commits `55f6872` impl + `ff3a757` resolve). Resumed from prior agent that exited prematurely after /relay-analyze; continuation agent completed /relay-superplan onward. Review APPROVED clean. Implement: 10 deviations applied + documented (highest of phase; producer-only ship with multiple spec-vs-current-state adjustments). Verify: 2 iterations + 1 fix. Suite 966 → 994 (+28). Summary: `.relay/.auto-session/2026-05-24-030632/57.json`.
- [x] 30.9 — `dual-driver-observer-advisor` (#56) — **SHIPPED 2026-05-24** via sweep session `2026-05-24-030632` (commits `ac7e37c` impl + `73d87cd` resolve). Review APPROVED clean (third in a row). Implement: 4 deviations. Verify: 1 iter + 1 fix. Suite 994 → 1025 (+31). Summary: `.relay/.auto-session/2026-05-24-030632/56.json`.
- [x] 30.10 — `dual-driver-halt-categories` (#61) — **SHIPPED 2026-05-24** via sweep session `2026-05-24-030632` (commits `ed1b974` impl + `5c37883` resolve). Review APPROVED clean (fourth in a row). Implement: 6 deviations. Verify: 1 iter + 1 fix. Suite 1025 → 1036 (+11). Summary: `.relay/.auto-session/2026-05-24-030632/61.json`.
- [x] 30.11 — `column-transition-op-triggering` (#50) — **SHIPPED 2026-05-24** via sweep session `2026-05-24-030632` (commits `b1a7abf` impl + `85b1bb6` resolve). Review APPROVED clean (fifth in a row). Implement: 1 deviation. Verify: 1 iter, 0 fixes. Suite 1036 → 1057 (+21). Summary: `.relay/.auto-session/2026-05-24-030632/50.json`.
- [x] 30.12 — `card-detail-run-history-surface` (#52) — **SHIPPED 2026-05-24** via sweep session `2026-05-24-030632` (commits `0fb4762` impl + `57a9cf1` resolve). Review APPROVED-WITH-CHANGES (3 trivial applied; streak of 5 clean APPROVEDs broken). Implement: 0 deviations. Verify: 1 iter, 0 fixes. Suite 1057 → 1068 (+11). Wave 1 of sweep complete (6 items shipped: #60, #57, #56, #61, #50, #52). Summary: `.relay/.auto-session/2026-05-24-030632/52.json`.
- [x] 30.13 — `dual-driver-brain-loop-replacement` (#59) **BIG-BANG SWITCH SHIPPED 2026-05-24** via sweep session `2026-05-24-030632` (5 commits: 4 impl `5e3429f`/`575c311`/`25a89a2`/`d4b0884` + 1 resolve `76be840`). Planner fell back from superplan → plan (same precedent as #54 in 30.2). Review APPROVED-WITH-CHANGES (5 trivial applied). Implement: 0 deviations (clean for the central switch). Verify: 1 iter, 0 fixes. Suite 1068 → 1085 (+17). **Architectural milestone**: `defaultAgentFactory` replaced with orchestrator-driven dispatch; brain == orchestrator running in tight iter loop; `Conductor` class public surface preserved. step_resolver decision: RETAIN (Phase 29.3 module stays). Summary: `.relay/.auto-session/2026-05-24-030632/59.json`.
- [x] 30.14 — `dual-driver-frame-b-chat-wire` (#62) — **SHIPPED 2026-05-24** via sweep session `2026-05-24-030632` (commits `5cbcebc` impl + `42d9880` resolve). Review APPROVED-WITH-CHANGES (3 trivial applied). Implement: 0 deviations. Verify clean. Suite 1085 → 1096 (+11). **Cross-cluster bridge milestone**: Frame B chat panel now invokes orchestrator-core decide() via new chat_command RPC + chat_classifier. Unblocks #49. Summary: `.relay/.auto-session/2026-05-24-030632/62.json`.
- [x] 30.15 — `chat-driven-description-authoring` (#49) **FINAL SWEEP ITEM SHIPPED 2026-05-24** via sweep session `2026-05-24-030632` (8 commits total: `62a7786` analyze+plan+review docs + 5 impl `e78fa5a`/`88cec85`/`32dd1ad`/`5a56a53`/`4384d09` + `25a6cdd` verify report + `c6ff57d` resolve; most-fragmented pattern of sweep). Planner fell back superplan → plan (same as #54 and #59). Review APPROVED-WITH-CHANGES (8 trivial — highest of sweep). Implement: 0 deviations. Verify clean. Suite 1096 → 1123 (+27). **Active feature backlog is now EMPTY**. Frame B Cohort B (convergence point) landed: ModelAdapter.invokeWithTools + chat_agent.ts (4-tool surface: grep/read/glob/propose-description-edit) + chat_apply_edit + chat_proposed_edit_get RPCs + diff-preview UI; v1 capped at 1 round of tool calls per spec. Summary: `.relay/.auto-session/2026-05-24-030632/49.json`.
...
**Next steps after 30.5 closes.** Phase 30 will have shipped 4 features (2 dual-driver + 2 Frame B). The "Phase 30 was scoped to kickoff + first foundation item" original framing has been generously exceeded. Strong recommendation: `/phase-close` after 30.5. Remaining backlog (~12 features) carries naturally into Phase 31+ at the established pace. If operator continues into 30.6+ instead, candidates would be dual-driver #58/#60 (foundation continuation) OR Frame B #50/#52 (Cohort C polish). **Resolved 2026-05-24**: operator chose to continue with 30.6 = dual-driver #58 (cross-cluster switch back to dual-driver foundation; #58 enables the #59 big-bang switch).

### 30.6 — Dual-driver foundation continued: `dual-driver-backward-transitions-and-substrate-advisory` (#58)

**Scope.** Widen the lifecycle state machine to allow all column→column edges (today the `BACKWARD` set is small per `src/conductor/lifecycle.ts`). Add substrate-aware advisory layer at transition time: when a backward edge would orphan substrate artifacts (e.g., moving `building → planned` leaves implement.md substrate without a corresponding plan in scope), surface a typed advisory with three choices — keep / wipe / branch. New RPCs for explicit substrate wipe + branch operations. Per `.relay/features/dual-driver-backward-transitions-and-substrate-advisory.md` for the full design.

**Why this item now.** Tied at 1 in-cluster dependent with #60 (`autonomy-spectrum-config`); file-order tie-break favors #58. Strategically, #58 is the harder of the two to defer: it enables the wider state machine that #59 brain-loop-replacement requires. Also addresses an open user-facing affordance that's been documented since Phase 14 ("No backward path from approved column" was a related issue then; #58 generalizes the fix).

**Dispatch path.** `/relay-auto` per priority. Per-item agent runs full pipeline. The Control bridge in CLAUDE.md:
1. Pre-dispatch: this step's row authored above. `/relay-auto` injects scope `(30.6)` into the per-item agent's commit subjects.
2. Post-dispatch: the bridge commit `docs(30.6): /relay-auto close out dual-driver-backward-transitions-and-substrate-advisory (commits: <agent-sha-list>)` flips this checkbox.

**Step-close commit:** flipped by `/relay-auto`'s bridge protocol post-agent-return.

**Cross-step coordination considerations.**
- Phase 14 (Board transition UX) already extracted `src/ui/views/board_validate.ts` with `FORWARD_MAP` + `BACKWARD_EDGES`. Widening the engine `BACKWARD` set requires mirroring in the UI validator (precedent: Phase 14 added `'approved->planned'` to BOTH simultaneously).
- Phase 17 #41 (`keyboard-board-focus-and-move`) imports `board_validate.ts`. Behavior should remain consistent across drag-drop AND keyboard-move paths.
- The substrate-advisory layer is NEW — design space includes UX choices for surfacing the keep/wipe/branch advisory (CLI prompt, UI modal, SSE event for the brain). The agent's /relay-plan should pin the v1 UX path.

**Next steps after 30.6 closes.** Phase 30 will have shipped 5 features (3 dual-driver + 2 Frame B). At that point the phase has overrun its original "kickoff + first foundation" scope by a wide margin. Strong-strong recommendation: `/phase-close` at 30.6. Remaining dual-driver foundation items (#60 autonomy-spectrum-config, #61 halt-categories) are independent and can ship in Phase 31+. Frame B Cohort C (#50, #52) is also unblocked and ready for Phase 31+.
