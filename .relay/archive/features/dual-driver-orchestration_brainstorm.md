...
> **ARCHIVED 2026-05-24** — All 9 features resolved (#1, #2, #3, #4, #5, #6, #7, #8, #9 all IMPLEMENTED). The dual-driver architecture is real; the cluster's vision (one orchestrator engine, two invocation surfaces) is realized end-to-end.

*Created: 2026-05-23*
*Status: COMPLETE*
(Lifecycle: BRAINSTORMING → READY FOR DESIGN → DESIGN COMPLETE → COMPLETE)
*Completed: 2026-05-24 via `/relay-resolve` for #9 (final cluster feature `dual-driver-frame-b-chat-wire`).*
...
|---|---|---|---|---|
| 1 | ~~[`dual-driver-orchestrator-core.md`](../archive/features/dual-driver-orchestrator-core.md)~~ ✓ **IMPLEMENTED 2026-05-24** ([impl](../implemented/dual-driver-orchestrator-core.md)) | LLM-driven orchestrator engine. Reads card state + substrate + halt log → decides next action → calls op as a tool. Single-card invocation surface (one orchestrator call = one decision). | Build first | None (foundation) |
| 2 | ~~[`dual-driver-lead-follow-protocol.md`](../archive/features/dual-driver-lead-follow-protocol.md)~~ ✓ **IMPLEMENTED 2026-05-24** ([impl](../implemented/dual-driver-lead-follow-protocol.md)) | **Global** lead tracking (brain on/off for whole board, not per-card); explicit lead-transfer mechanisms (CLI + UI + user chat); typed events (single `lead-handed-off` variant carrying previous + current state). Runtime store field + RPC. **Subsumed Frame B Feature #5 (`brain-halt-on-user-chat`)** under the global-lead model. Closes the #54 v1 `lead='human'` hardcode. | Build second | #1 (orchestrator-core decides when to publish `halt-with-handoff`) |
| 3 | ~~[`dual-driver-observer-advisor.md`](../archive/features/dual-driver-observer-advisor.md)~~ ✓ **IMPLEMENTED 2026-05-24** ([impl](../implemented/dual-driver-observer-advisor.md)) | Non-lead watches lead's actions; publishes typed advisory events on detected out-of-sequence actions. Same orchestrator-core engine, different invocation pattern (read-only + advisory-emit-only). Note: under global-lead model, "observer" runs only on individual actions during the lead's session (substrate-aware advisories at transition time, etc.) — the big reconciliation happens at lead-handoff via #4. | Build third | #1, #2 |
| 4 | ~~[`dual-driver-lead-handoff-reconciliation.md`](../archive/features/dual-driver-lead-handoff-reconciliation.md)~~ ✓ **IMPLEMENTED 2026-05-24** ([impl](../implemented/dual-driver-lead-handoff-reconciliation.md)) | When brain reclaims lead: diff board state since handoff; re-evaluate prior plans/decisions per affected card; surface reconciliation summary; bounded by cost ceiling. **First-class feature** per operator's decision #8 — the brain doesn't just resume, it re-thinks. | Build fourth | #1, #2 (foundation + lead state to detect handoff transitions) |
| 5 | ~~[`dual-driver-backward-transitions-and-substrate-advisory.md`](../archive/features/dual-driver-backward-transitions-and-substrate-advisory.md)~~ ✓ **IMPLEMENTED 2026-05-24** ([impl](../implemented/dual-driver-backward-transitions-and-substrate-advisory.md)) | Widen state machine to allow all column→column edges. Substrate-aware advisory layer at transition time: keep / wipe / branch choices for transitions that would orphan substrate. New RPC for explicit substrate wipe + branch operations. | Build fifth | None directly (state machine surface); integrates with #1 (orchestrator decisions reference advisory categories) |
| 6 | ~~[`dual-driver-brain-loop-replacement.md`](../archive/features/dual-driver-brain-loop-replacement.md)~~ ✓ **IMPLEMENTED 2026-05-24** ([impl](../implemented/dual-driver-brain-loop-replacement.md)) | Replace the current Conductor loop with orchestrator-core running in a tight iter. Removes `defaultAgentFactory`'s hardcoded TaskAgent shape; orchestrator decides per-card per-iter. Includes halt-loop circuit breaker. | Build sixth | #1, #2, #4 (lead state + reconciliation pass needed on first iter after lead-acquired) |
| 7 | ~~[`dual-driver-autonomy-spectrum-config.md`](../archive/features/dual-driver-autonomy-spectrum-config.md)~~ ✓ **IMPLEMENTED 2026-05-24** ([impl](../implemented/dual-driver-autonomy-spectrum-config.md)) | Replace `autonomy.transitions.*` config with `autonomy: assist \| hybrid \| autonomous` (project default + per-card override). Wire to orchestrator-core's decision policy. Migration path for existing configs. | Can ship parallel with #1–#6 | #1 |
| 8 | ~~[`dual-driver-halt-categories.md`](../archive/features/dual-driver-halt-categories.md)~~ ✓ **IMPLEMENTED 2026-05-24** ([impl](../implemented/dual-driver-halt-categories.md)) | Named recovery-categories in `classifyHalt()`; orchestrator dispatches on categories; observer-advisor uses categories for out-of-sequence detection. | Can ship anytime ≥ #1 | #1 (lighter dependency) |
| 9 | ~~[`dual-driver-frame-b-chat-wire.md`](../archive/features/dual-driver-frame-b-chat-wire.md)~~ ✓ **IMPLEMENTED 2026-05-24** ([impl](../implemented/dual-driver-frame-b-chat-wire.md)) | Frame B's chat panel calls orchestrator-core for "what's next for this card?" / "diagnose this halt" / "advance this card to <column>". Same engine, UI-driven invocation. Note: under global-lead model, UI chat is a SURFACE the operator-as-lead uses; it doesn't shift lead per-card. | Build alongside Frame B | #1 (foundation); blocks Frame B's full UX |
