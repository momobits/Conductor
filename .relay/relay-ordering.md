...
| 2 | `scan` continues on per-card YAML failure (warns, exits 0 if any healthy) | [scan-bails-entirely-on-one-malformed-card.md](issues/scan-bails-entirely-on-one-malformed-card.md) | M | #1 (uses typed errors) ✓ available |
