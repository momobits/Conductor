...
## Phase 13 — Routing config destructiveness (cluster) — PR-1 COMPLETE; PR-2 ACTIVE

**PR-1 resolved:** 2026-05-16 — shipped as Control phase 22.1 (grouped run of 3 entries: #25 + #26 + #28; commits c22cb0c + 9053529 + cc86027 + 9c2a8f6). See [implementation doc](implemented/ui-routing-yaml-commit-silently-resets-omitted-fields-to-defaults.md).

**PR-1 resolution summary.** Server-side deep-merge in `config_set` (with critical /relay-review fix: bypass `ConfigSetParams.parse` which would otherwise fill defaults and defeat the merge); shape-check via `z.record(z.unknown())`; ENOENT-guarded `loadProjectConfig`; new `deepMergeConfig` + `isPlainObject` helpers; re-validate via strict schema. Schema preprocess `null → Infinity` on `cost_ceilings.per_card_dollars` + `per_day_dollars` makes JSON roundtrip transparent. HTTP RPC error handler formats `ZodError.issues` as `<path>: <msg>` joined by `; ` (top-level refines → `(root):`); structured issues exposed in `error.data.issues` for programmatic clients. Suite 585 → 596 (+11 net new tests).

**PR-2 (#24 + #27) remains ACTIVE** — depends on PR-1's merge-aware `config_set`. See Phase 23 in the recommended next-up cluster.

| #  | Item                                                                                       | File                                                                                                                            | Complexity | Depends on                                          |
|----|-------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------|------------|-----------------------------------------------------|
| 24 | Routing autonomy dropdown silently overwrites uncommitted yaml edits                       | [ui-routing-autonomy-dropdown-overwrites-uncommitted-yaml-edits.md](issues/ui-routing-autonomy-dropdown-overwrites-uncommitted-yaml-edits.md) | S      | #25 ✓ (merge-aware config_set now available for surgical update) |
